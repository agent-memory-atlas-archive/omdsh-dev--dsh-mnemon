...
it('previews exact descriptor-owned settings before saving and invalidates an edited preview', async () => {
  ...
  await screen.findByLabelText('配置已安装组件')
  fireEvent.change(screen.getByLabelText('配置已安装组件'), { target: { value: 'focus' } })
  fireEvent.click(screen.getByLabelText('启用此增强'))
...
it('discards an old session dashboard after the settings scope changes', async () => {
  ...
  await screen.findByRole('option', { name: '当前配置 · 已关闭' })
  old.resolve({ ok: true, value: dashboard })
  await waitFor(() => expect(screen.queryByRole('option', { name: '专注上下文 · 已关闭' })).toBeNull())
