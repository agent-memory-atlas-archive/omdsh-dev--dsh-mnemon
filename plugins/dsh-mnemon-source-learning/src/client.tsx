...
import { installMemorySourceUI, MemoryPluginMetrics, MemoryPluginNotice, MemoryPluginSurface, MemorySourcePageFrame, type MemorySourcePageProps, type MemorySourceUIContext } from 'dsh-mnemon/client'
...
export function Page(props: MemorySourcePageProps) {
  ...
  useEffect(() => setLimit(30), [tab, props.workspaceId, props.sessionId])
  ...
  const write = async (operation: string, input: Fields, message = t.saved) => {
    ...
    try {
      ...
      setSnapshot(result.value as unknown as Snapshot); setNotice(message); props.onRefresh?.(); return true
  ...
  return <MemorySourcePageFrame locale={props.locale}><MemoryPluginSurface title={t.title} description={t.description} actions={<><button disabled={busy} onClick={() => void load()}>{t.refresh}</button>{' '}<button data-primary="true" disabled={!props.writable || busy || running || newEvidence < 1} onClick={() => void write('review-now', {}, t.running)}>{running ? t.running : t.review}</button></>}>
    <MemoryPluginMetrics items={[{ label: t.rounds, value: rounds }, { label: t.proposals, value: proposals.filter(r => r.state === 'pending').length }, { label: t.adopted, value: proposals.filter(r => r.state === 'active' || r.data.destinationState === 'active').length }, { label: t.concerns, value: proposals.filter(r => r.data.needsReview === true).length }]} />
    {!cycle && <p>{t.chooseSession}</p>}{error && <MemoryPluginNotice error>{error}</MemoryPluginNotice>}{notice && <MemoryPluginNotice>{notice}</MemoryPluginNotice>}{cycle?.data.status === 'failed' && <MemoryPluginNotice error>{String(cycle.data.error)}</MemoryPluginNotice>}
    ...
    <div className="mc-list">{rows.slice(0, limit).map(record => {
      ...
      return <article key={record.id} data-record-id={record.id}>
        <h3>{record.title}</h3><div className="mc-meta"><span className="mc-badge">{isProposal ? category(record) : String(record.data.origin ?? record.data.kind ?? record.kind)}</span><time dateTime={record.updatedAt}>{new Date(record.updatedAt).toLocaleString(props.locale)}</time>{isProposal && <><span>{t[record.scope as 'global' | 'project']}</span><span>{record.signals} {t.signals}</span></>}</div>
        {record.data.needsReview === true && <MemoryPluginNotice>{t.needsReview}</MemoryPluginNotice>}
        {editing === record.id ? <form onSubmit={event => { event.preventDefault(); void write('update', { id: record.id, version: record.version, title, content }).then(ok => { if (ok) setEditing('') }) }}><label>{t.titleField}<input required value={title} onChange={event => setTitle(event.target.value)} /></label><label>{t.content}<textarea required rows={7} value={content} onChange={event => setContent(event.target.value)} /></label><footer><button data-primary="true" disabled={busy}>{t.save}</button><button type="button" onClick={() => setEditing('')}>{t.cancel}</button></footer></form> : <p className="mc-content">{record.content}</p>}
        {isProposal && <>
          <details><summary>{t.showEvidence} ({Array.isArray(record.data.evidenceIds) ? record.data.evidenceIds.length : 0})</summary>{snapshot.records.filter(r => Array.isArray(record.data.evidenceIds) && record.data.evidenceIds.includes(r.id)).map(evidence => <div key={evidence.id}><small>{String(evidence.data.origin)} · {new Date(evidence.createdAt).toLocaleString(props.locale)}</small><p className="mc-content">{evidence.content}</p></div>)}</details>
          {record.state === 'pending' && record.data.category === 'preference' && humanSignals < 2 && <p><small>{t.waitSignals}</small></p>}
          {record.data.destination && <p><small>{record.data.destinationState === 'active' ? t.targetActive : t.targetPending}</small></p>}
