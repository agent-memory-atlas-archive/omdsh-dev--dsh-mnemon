...
import { json, newRecord, reviseRecord, visibleRecord, type RecordSnapshot } from 'dsh-mnemon-workspace-kit'
...
export function reviewPrompt(window: LearningWindow): string {
  return JSON.stringify({ token: window.token, throughRound: window.throughRound,
    evidence: window.evidence.map(record => ({ id: record.id, origin: record.data.origin, independentKey: record.data.independentKey, content: record.content, ...(record.data.redacted === true ? { redacted: true } : {}), ...(record.data.excerpt === true ? { excerpt: true } : {}) })),
