...
Install `dsh-mnemon-source-files` alongside `dsh-mnemon`. Enable a Strategy supporting `file-search`, such as `dsh-mnemon-strategy-workspace`. Optional configuration: `dataDir`, `roots` (absolute directories), and `rgPath` (defaults to `rg`). No implicit home-directory scan takes place.
...
安装本包与 `dsh-mnemon`，启用支持 `file-search` 的策略即可。可选配置为 `dataDir`、绝对目录数组 `roots` 和 `rgPath`。此插件不会隐式扫描用户主目录。页面结果出现截断提示时，应缩小范围后继续检索。
