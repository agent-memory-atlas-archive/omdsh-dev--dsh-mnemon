...
it('bases timing on Source facts and explicitly configurable feedback policies', () => {
  ...
  expect(evaluate({ newHumanTurns: 4 }).guidance?.system).not.toContain('Learning due Sources')
  expect(evaluate({ newHumanTurns: 5 }).guidance?.system).toContain('Learning due Sources: source:learning')
  expect(evaluate({ newFeedback: 1 }).guidance?.system).toContain('Learning due Sources')
  expect(evaluate({ newOutcomes: 2 }).guidance?.system).toContain('Learning due Sources')
