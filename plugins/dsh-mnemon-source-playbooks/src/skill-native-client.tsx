...
export function NativeSkills(props: MemorySourcePageProps & { onNotice(value: string): void; onError(value: string): void; onDirty(value: boolean): void }) {
  const zh = props.locale.startsWith('zh'), t = (en: string, cn: string) => zh ? cn : en
  const [entries, setEntries] = useState<NativeEntry[]>([]), [roots, setRoots] = useState<string[]>([]), [root, setRoot] = useState(''), [revision, setRevision] = useState(''), [selected, setSelected] = useState<NativeDetail>(), [files, setFiles] = useState<NativeDetail['files']>([]), [query, setQuery] = useState(''), [busy, setBusy] = useState(false), [instruction, setInstruction] = useState('')
  ...
  async function load(name?: string) {
    ...
    try {
      ...
      else { const value = result.value as unknown as { skills: NativeEntry[]; roots: string[] }; setEntries(value.skills); setRoots(value.roots) }
  ...
  async function mutate(operation: string, input: Record<string, MemoryJsonValue>) {
    ...
    try {
      ...
      await props.management.mutate(operation, input, { confirmed: true, expectedRevision: current.revision })
      if (!requests.isCurrent(token)) return
      if (selected && ['skills-native-save', 'skills-native-toggle'].includes(operation)) {
        ...
        if (!requests.isCurrent(token)) return
        setSelected(detail); setFiles(detail.files); setRevision(result.revision)
      }
      const result = await props.management.read('skills-catalog', {}), value = result.value as unknown as { skills: NativeEntry[]; roots: string[] }
      if (!requests.isCurrent(token)) return
      setEntries(value.skills); setRoots(value.roots); setRevision(result.revision); setRoot(''); props.onNotice(t('Native skill settings saved.', '原生技能设置已保存。'))
    } catch (error) { if (requests.isCurrent(token)) props.onError(managementError(error, zh)) }
    finally { if (requests.isCurrent(token)) { busyRef.current = false; setBusy(false) } }
  }
  ...
  return <div style={{ marginTop: 18 }}>
    ...
    <details><summary>{t('Additional skill directories', '附加技能目录')} ({roots.length})</summary><p>{t('Directories are discovered by the native provider. Removing a directory only disconnects it; its files stay in place.', '目录由原生 Provider 发现。移除目录只解除接入，文件保留原位。')}</p>{roots.map(path => <div key={path} className="mc-meta"><code style={{ overflowWrap: 'anywhere' }}>{path}</code><button disabled={disabled || dirty} onClick={() => void mutate('skills-remove-root', { path })}>{t('Disconnect', '解除接入')}</button></div>)}<label>{t('Directory path', '目录路径')}<input value={root} onChange={event => setRoot(event.target.value)} placeholder="/path/to/skills" /></label><button disabled={disabled || dirty || !root.trim()} onClick={() => void mutate('skills-add-root', { path: root })}>{t('Connect directory', '接入目录')}</button></details>
