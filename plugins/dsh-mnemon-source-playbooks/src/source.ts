...
export const sourceOptions: RecordSourceOptions = {
  ...
  kinds: ['skill', 'prompt'], scopes: ['global', 'project'], defaultScope: 'project',
  prepare(record) { record.data.enabled ??= true; record.data.category ??= ''; record.data.uses ??= 0 },
  validate(record) {
  ...
  project(records) { return 'Approved playbooks (read the full content before use):\n' + records.filter(record => record.data.enabled !== false).map(record => `${record.id} · ${record.kind} · ${record.title}${record.data.category ? ' · ' + String(record.data.category) : ''}`).join('\n') },
