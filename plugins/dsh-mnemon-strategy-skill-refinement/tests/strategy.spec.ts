...
it('only prompts refinement for sufficient evidence or enabled feedback policy', () => {
  const evaluate = (signals: number[], feedback: number, config = {}) => { const extension = memoryStrategyConfiguration.create(config).strategyExtensions![0]!, sources = [source(signals, feedback)]; return WORKSPACE_STRATEGY.compose(request, sources, [{ instanceKey: 'extension:skills', typeId: 'skill-refinement', slot: 'skills', value: extension.contribute(request, sources) }]).guidance?.system }
  expect(evaluate([1], 0)).not.toContain('Skill refinement Sources')
  expect(evaluate([2], 0)).toContain('Skill refinement Sources: source:playbooks')
  expect(evaluate([], 1)).toContain('Skill refinement Sources')
  expect(evaluate([], 1, { reviewFeedback: false })).not.toContain('Skill refinement Sources')
  expect(evaluate([2], 0)).toContain('Do not execute candidate scripts')
...
it('declares the slot accepted by the selected Workspace strategy', () => {
  ...
  expect(WORKSPACE_STRATEGY.manifest.extensionSlots).toContain(extension.manifest.slot)
