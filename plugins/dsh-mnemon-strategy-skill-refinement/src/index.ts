...
export const memoryStrategyConfiguration = defineMemoryStrategyConfiguration({ kind: 'strategy-extension', typeId: 'skill-refinement', label: memoryPlugin.label, description: memoryPlugin.description,
  ...
  create: config => ({ plugin: memoryPlugin, strategyExtensions: [defineWorkspacePolicy({ typeId: 'skill-refinement', packageName: name, slot: 'skills', contribute: () => ({ instruction: typeof config.instruction === 'string' ? config.instruction : instruction, minimumSignals: typeof config.minimumSignals === 'number' ? config.minimumSignals : 2, reviewFeedback: config.reviewFeedback !== false }) })] }),
})
