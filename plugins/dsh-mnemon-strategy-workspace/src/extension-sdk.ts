...
export function validateWorkspacePolicy<K extends WorkspacePolicySlot>(slot: K, value: MemoryJsonValue): WorkspacePolicies[K] {
  ...
  const allowed = slot === 'focus' ? ['sourceKeys', 'writableSourceKeys', 'maxProjectionCharacters'] : slot === 'review' ? ['interval', 'instruction'] : ['capture', 'prompts', 'collaboration'].includes(slot) ? ['instruction'] : []
